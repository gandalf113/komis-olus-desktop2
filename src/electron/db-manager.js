...
getClientsContracts = ipcMain.handle(
...
getReturns = ipcMain.handle("get/returns", async (event, args) => {
  ...
  return knex
    ...
    .leftOuterJoin(
      "przedmioty",
      "zwroty.id_przedmiotu",
      "przedmioty.id_przedmiotu"
    )
...
getContractsWithClients = ipcMain.handle(
  "get/contracts-clients",
  async (event, args) => {
    const { contractId } = args;
    ...
    let query = knex
      .select("*")
      .from("umowy")
      .leftOuterJoin("klienci", "klienci.id_klienta", "umowy.id_klienta");
...
getSalesWithItemsByDate = ipcMain.handle(
  "get/sales-items/date",
  async (event, args) => {
    const { date } = args;
    ...
    return knex
      ...
      .leftJoin("klienci", "umowy.id_klienta", "klienci.id_klienta")
      .orderBy("id_sprzedazy", "desc")
      .where("sprzedaz.data", date);
  }
);

getItemsWithContracts = ipcMain.handle(
  "get/items-contracts",
  async (event, args) => {
    const { contractId } = args;
    return knex
      .select("*")
      .from("przedmioty")
      .leftOuterJoin("umowy", "umowy.id_umowy", "przedmioty.id_umowy")
      .where("przedmioty.id_umowy", contractId);
  }
);

getClientsWithContractsAndItems = ipcMain.handle(
  "get/clients-contracts-items",
  async (event, args) => {
    const { search } = args;
...
getItemsForGivenClient = ipcMain.handle(
  "get/client/items",
  async (event, args) => {
    const { clientId } = args;
...
getWithdrawsForGivenClient = ipcMain.handle(
...
searchClients = ipcMain.handle("search/clients", async (event, args) => {
  ...
  return knex.select("*").from("klienci").whereLike("skrot", `%${search}%`);
});

searchClientExact = ipcMain.handle(
  "search/clients/exact",
  async (event, args) => {
    const { search } = args;
...
updateClient = ipcMain.handle("update/client", async (event, args) => {
  ...
  return knex("klienci").where({ id_klienta: clientId }).update({
    imie: firstName,
    nazwisko: lastName,
    skrot: short,
    adres: address,
...
updateSale = ipcMain.handle("update/sale", async (event, args) => {
  ...
  return knex("sprzedaz").where({ id_sprzedazy: saleId }).update({
    marza: margin,
    cena: price,
    data_sprzedazy: date,
  });
...
updateItem = ipcMain.handle("update/item", async (event, args) => {
  const { itemId, name, commiterValue, defaultMargin, amount, soldAmount } =
    args;

  return knex("przedmioty").where({ id_przedmiotu: itemId }).update({
    nazwa: name,
    kwotaDlaKomitenta: commiterValue,
    domyslnaMarza: defaultMargin,
    przyjetaIlosc: amount,
    sprzedanaIlosc: soldAmount,
  });
});
...
updateContract = ipcMain.handle("update/contract", async (event, args) => {
  ...
  return knex("umowy").where({ id_umowy: contractId }).update({
    id_klienta: clientId,
    numer_umowy: contractNumber,
  });
});

incrementSoldAmount = ipcMain.handle(
  "increment/soldAmount",
  async (event, args) => {
    const { itemId } = args;
...
incrementReturnedAmountBy = ipcMain.handle(
...
decrementReturnedAmountBy = ipcMain.handle(
  "decrement/returnedAmount",
  async (event, args) => {
    const { itemId, amount } = args;
...
deleteClient = ipcMain.handle("delete/client", async (event, args) => {
  ...
  return knex.select("*").from("klienci").where("id_klienta", clientId).del();
});
...
deleteContract = ipcMain.handle("delete/contract", async (event, args) => {
  ...
  return knex.select("*").from("umowy").where("id_umowy", contractId).del();
});
...
deleteWithdraw = ipcMain.handle("delete/withdraw", async (event, args) => {
  ...
  return knex.select("*").from("wyplaty").where("id_wyplaty", withdrawId).del();
});
...
deleteReturn = ipcMain.handle("delete/return", async (event, args) => {
  ...
  return knex.select("*").from("zwroty").where("id_zwrotu", returnId).del();
});
